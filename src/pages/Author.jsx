    ...
    <div className="min-h-screen bg-[#0E1217]">
      <div className="container mx-auto px-4 pt-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {/* Il faut utiliser map pour générer plusieurs cartes, ici c'est pour afficher une carte pour chaque auteur */}
          {authors.map((auteurs) => (
            <Card key={auteurs.id} className="max-w-sm mx-auto my-4 bg-[#1A202C] transition duration-500 ease-in-out transform hover:-translate-y-1 hover:shadow-xl">
              <CardBody className="flex flex-col items-center">
                <div className="w-24 h-24 mb-4">
                  {/* On affiche l'image de l'auteur dans la src et le nom dans le alt */}
                  <img src={auteurs.authorImageUrl} alt={auteurs.author} className="rounded-full shadow-lg" />
                </div>
                <Typography variant="h5" className="mb-2 text-center text-[#E11D48]">
                  {auteurs.author} {/* On affiche le nom de l'auteur */}
                </Typography>
              </CardBody>
